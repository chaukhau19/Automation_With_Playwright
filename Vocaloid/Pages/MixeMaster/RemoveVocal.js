...
async VerifyLocator(expectedLocator) {
  try {
      ...
      await expect(expectedLocator).toBeVisible({ timeout: 200000 });
...
async clickConvertVoiceButton(convertVoiceButton) {
try {
  const buttonVisible = await convertVoiceButton.isVisible();
  const clickCount = 2; 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  async removeVocalbyLink() {
    try {
      ...
      await this.page.getByPlaceholder('Paste a Youtube link...').fill(config.Youtube_8);
      
      await this.ClickNTime(removeVocalButton, 2, "✅ Clicking Generate...", "❌ Generate button is not present.");
...
async DeleteHistory() {
  ...
  const deleteButton = this.page.locator(`//tr[.//h6[text()='${config.Youtube_Text_8}']][1]//button[contains(@class, 'delete-button')]`);
  ...
  await this.page.waitForSelector(`(//h6[text()='${config.Youtube_Text_8}'])[1]`, { state: 'visible' });
